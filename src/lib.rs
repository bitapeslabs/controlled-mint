...
use alkanes_support::context::Context as AlkanesContext;
...
impl ControlledMint {
    ...
    fn get_owner_id(&self) -> Result<SchemaAlkaneId> {
        ...
        let alkane_id = SchemaAlkaneId::deserialize_reader(&mut byte_reader)
            .map_err(|_| anyhow!("CONTROLLED MINT: Failed to decode owner at get_owner_id"))?;
        Ok(alkane_id)
    }

    fn assert_owner(&self, context: &AlkanesContext) -> Result<()> {
        let owner = self.get_owner_id()?;

        ensure!(
            context.caller.block == owner.block.into() && context.caller.tx == owner.tx.into(),
            "CONTROLLED MINT: Caller is not the owner"
        );
...
impl ControlledMint {
    fn initialize(&self) -> Result<CallResponse> {
        ...
        let consts =
            SchemaControlledMintInitializationParameters::deserialize_reader(&mut byte_reader)
                .map_err(|_| {
                    anyhow!("CONTROLLED MINT: Failed to decode initialization parameters")
                })?;
    ...
    pub fn change_owner(&self) -> Result<CallResponse> {
        let context = self.context()?;
        self.assert_owner(&context)?;
        let response = CallResponse::forward(&context.incoming_alkanes);

        let new_owner = decode_from_ctx!(context, SchemaAlkaneId).context(
            "CONTROLLED MINT: failed to change owner because of serialization failure of params",
        )?;

        let new_owner_bytes =
            borsh::to_vec(&new_owner).context("CONTROLLED MINT: Failed to serialize new owner")?;

        self.get_owner_pointer().set(Arc::new(new_owner_bytes));
        Ok(response)
    }
    ...
    pub fn renounce_ownership(&self) -> Result<CallResponse> {
        ...
        //Here we set DIESEL to the new owner
